...
3. **事实准确（A，联网，重点）**
   ...
   - 核对量：数据密集型文件 10-15 个最关键事实，叙述型文件 5-8 个（--all 批量模式为 3-5，见下）
...
报告完成后询问用户：是否按修复清单直接修复原文件？确认后执行修复（修正错误数字、补缺失知识点、补 frontmatter），并更新 `collected` 为今日。非交互场景（subagent/批量调用）跳过询问，只出报告不修复。

...
1. **扫 raw_material frontmatter**（跳过 `_audit/` 和 `_meta/`）：
   - 🔴 已过期：`valid_until` < 今天
   - 🟡 即将过期：今天 ≤ `valid_until` < 今天 + 30 天（边界例：valid_until 恰为今天 → 🟡，不算过期）
   - ⚪ 标注异常：缺 freshness；annual/volatile 却缺 valid_until；**evergreen 却填了 valid_until**（反向错误，也算 ⚪）
   - 🟢 有效：其余（报告中只计数，不列明细）

2. **跨层关联**：对每个 🔴/🟡 文件，Grep wiki 的 `source:` 字段找出引用它的 wiki 条目。匹配方式：按 raw_material 文件名（不含 .md）做子串匹配——source 可能写成 `[[raw_material/分类/文件名]]` 单链或多链列表，子串匹配两种都能覆盖
...
- ⚪ 标注异常文件提示补/修 frontmatter（可转 /wealth-audit 处理）
- 不做 cron 自动化，手动触发
